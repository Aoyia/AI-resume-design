...
## 任务 4：更新主页面 page.tsx 实施静默预载

**文件：**
- 修改：`src/app/page.tsx`

- [ ] **步骤 1：移除全屏 checking 遮罩，修改为逻辑组件防抖渲染**
...
修改为直接渲染，并依据 `authorized && !checking` 过滤工具栏：
...
- [ ] **步骤 2：对 aside 面板和 PreviewPanel 的授权逻辑进行优化**

更新左侧编辑 aside 遮罩与 PreviewPanel 的状态绑定，合并 `!checking` 状态：
...
git commit -m "fix: 主页面 page.tsx 实施静默预载机制，彻底消除白屏瞬闪"
```
